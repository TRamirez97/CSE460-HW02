  ...
  *ps = s;
  ...
  if(eq)
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     53e:	50                   	push   %eax
     53f:	68 e8 18 00 00       	push   $0x18e8
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     560:	56                   	push   %esi
     561:	68 f0 18 00 00       	push   $0x18f0
...
nulterminate(struct cmd *cmd)
{
  ...
  switch(cmd->type){
     ...
     9d9:	ff 24 85 f8 12 00 00 	jmp    *0x12f8(,%eax,4)
...
printint(int fd, int xx, int base, int sgn)
{
     ...
     e14:	56                   	push   %esi
     e15:	53                   	push   %ebx
     e16:	83 ec 3c             	sub    $0x3c,%esp
     e19:	89 4d c4             	mov    %ecx,-0x3c(%ebp)
  uint x;
  ...
  if(sgn && xx < 0){
    neg = 1;
    x = -xx;
     e1c:	89 d1                	mov    %edx,%ecx
{
     e1e:	89 45 b8             	mov    %eax,-0x48(%ebp)
  if(sgn && xx < 0){
     e21:	85 d2                	test   %edx,%edx
     e23:	0f 89 7f 00 00 00    	jns    ea8 <printint+0x98>
     e29:	f6 45 08 01          	testb  $0x1,0x8(%ebp)
     e2d:	74 79                	je     ea8 <printint+0x98>
    neg = 1;
     e2f:	c7 45 bc 01 00 00 00 	movl   $0x1,-0x44(%ebp)
    x = -xx;
     e36:	f7 d9                	neg    %ecx
  } else {
    x = xx;
  ...
  i = 0;
     e38:	31 db                	xor    %ebx,%ebx
     e3a:	8d 75 d7             	lea    -0x29(%ebp),%esi
  ...
  do{
    buf[i++] = digits[x % base];
     ...
     e46:	f7 75 c4             	divl   -0x3c(%ebp)
     e49:	0f b6 92 18 13 00 00 	movzbl 0x1318(%edx),%edx
     e50:	89 45 c0             	mov    %eax,-0x40(%ebp)
     e53:	89 d8                	mov    %ebx,%eax
     e55:	8d 5b 01             	lea    0x1(%ebx),%ebx
  }while((x /= base) != 0);
     e58:	8b 4d c0             	mov    -0x40(%ebp),%ecx
    buf[i++] = digits[x % base];
     e5b:	88 14 1e             	mov    %dl,(%esi,%ebx,1)
  }while((x /= base) != 0);
     e5e:	39 7d c4             	cmp    %edi,-0x3c(%ebp)
     e61:	76 dd                	jbe    e40 <printint+0x30>
  if(neg)
     e63:	8b 4d bc             	mov    -0x44(%ebp),%ecx
     e66:	85 c9                	test   %ecx,%ecx
     e68:	74 0c                	je     e76 <printint+0x66>
    buf[i++] = '-';
     e6a:	c6 44 1d d8 2d       	movb   $0x2d,-0x28(%ebp,%ebx,1)
    buf[i++] = digits[x % base];
     e6f:	89 d8                	mov    %ebx,%eax
    buf[i++] = '-';
     e71:	ba 2d 00 00 00       	mov    $0x2d,%edx

  while(--i >= 0)
     e76:	8b 7d b8             	mov    -0x48(%ebp),%edi
     e79:	8d 5c 05 d7          	lea    -0x29(%ebp,%eax,1),%ebx
     e7d:	eb 07                	jmp    e86 <printint+0x76>
     e7f:	90                   	nop
  ...
  write(fd, &c, 1);
     ...
     e8e:	56                   	push   %esi
     e8f:	57                   	push   %edi
     e90:	e8 be fe ff ff       	call   d53 <write>
  while(--i >= 0)
     e95:	83 c4 10             	add    $0x10,%esp
     e98:	39 de                	cmp    %ebx,%esi
     e9a:	75 e4                	jne    e80 <printint+0x70>
    putc(fd, buf[i]);
}
  ...
  neg = 0;
     ea8:	c7 45 bc 00 00 00 00 	movl   $0x0,-0x44(%ebp)
     eaf:	eb 87                	jmp    e38 <printint+0x28>
     eb1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
     eb8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
     ebf:	90                   	nop
...
printf(int fd, char *fmt, ...)
{
  ...
  for(i = 0; fmt[i]; i++){
     ...
     ed1:	0f 84 b8 00 00 00    	je     f8f <printf+0xcf>
  ap = (uint*)(void*)&fmt + 1;
  ...
  write(fd, &c, 1);
     ...
     f09:	e8 45 fe ff ff       	call   d53 <write>
     f0e:	8b 55 d4             	mov    -0x2c(%ebp),%edx
      } else {
        putc(fd, c);
     f11:	83 c4 10             	add    $0x10,%esp
  for(i = 0; fmt[i]; i++){
     f14:	0f b6 1e             	movzbl (%esi),%ebx
     f17:	83 c6 01             	add    $0x1,%esi
     f1a:	84 db                	test   %bl,%bl
     f1c:	74 71                	je     f8f <printf+0xcf>
    c = fmt[i] & 0xff;
     f1e:	0f be cb             	movsbl %bl,%ecx
     f21:	0f b6 c3             	movzbl %bl,%eax
    if(state == 0){
     f24:	85 d2                	test   %edx,%edx
     f26:	74 c8                	je     ef0 <printf+0x30>
      }
    } else if(state == '%'){
     f28:	83 fa 25             	cmp    $0x25,%edx
     f2b:	75 e7                	jne    f14 <printf+0x54>
      if(c == 'd'){
     f2d:	83 f8 64             	cmp    $0x64,%eax
     f30:	0f 84 9a 00 00 00    	je     fd0 <printf+0x110>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
     f36:	81 e1 f7 00 00 00    	and    $0xf7,%ecx
     f3c:	83 f9 70             	cmp    $0x70,%ecx
     f3f:	74 5f                	je     fa0 <printf+0xe0>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
     f41:	83 f8 73             	cmp    $0x73,%eax
     f44:	0f 84 d6 00 00 00    	je     1020 <printf+0x160>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
     ...
     f4a:	83 f8 63             	cmp    $0x63,%eax
     f4d:	0f 84 8d 00 00 00    	je     fe0 <printf+0x120>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
     f53:	83 f8 25             	cmp    $0x25,%eax
     f56:	0f 84 b4 00 00 00    	je     1010 <printf+0x150>
  write(fd, &c, 1);
     f5c:	83 ec 04             	sub    $0x4,%esp
     f5f:	c6 45 e7 25          	movb   $0x25,-0x19(%ebp)
     f63:	6a 01                	push   $0x1
     f65:	57                   	push   %edi
     f66:	ff 75 08             	pushl  0x8(%ebp)
     f69:	e8 e5 fd ff ff       	call   d53 <write>
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
     f6e:	88 5d e7             	mov    %bl,-0x19(%ebp)
  write(fd, &c, 1);
     f71:	83 c4 0c             	add    $0xc,%esp
     f74:	6a 01                	push   $0x1
     f76:	83 c6 01             	add    $0x1,%esi
     f79:	57                   	push   %edi
     f7a:	ff 75 08             	pushl  0x8(%ebp)
     f7d:	e8 d1 fd ff ff       	call   d53 <write>
  for(i = 0; fmt[i]; i++){
     f82:	0f b6 5e ff          	movzbl -0x1(%esi),%ebx
        putc(fd, c);
  ...
  for(i = 0; fmt[i]; i++){
     f8b:	84 db                	test   %bl,%bl
     f8d:	75 8f                	jne    f1e <printf+0x5e>
    }
  }
}
     f8f:	8d 65 f4             	lea    -0xc(%ebp),%esp
     f92:	5b                   	pop    %ebx
     f93:	5e                   	pop    %esi
     f94:	5f                   	pop    %edi
     f95:	5d                   	pop    %ebp
     f96:	c3                   	ret    
     f97:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
     ...
     fb2:	e8 59 fe ff ff       	call   e10 <printint>
        ap++;
  ...
  write(fd, &c, 1);
     ff4:	e8 5a fd ff ff       	call   d53 <write>
        ap++;
  ...
  write(fd, &c, 1);
    ...
    1016:	e9 59 ff ff ff       	jmp    f74 <printf+0xb4>
  ...
  write(fd, &c, 1);
    ...
    106b:	57                   	push   %edi
    106c:	53                   	push   %ebx
    106d:	e8 e1 fc ff ff       	call   d53 <write>
        while(*s != 0){
...
malloc(uint nbytes)
{
  ...
  return freep;
    11a7:	a1 64 19 00 00       	mov    0x1964,%eax
      if((p = morecore(nunits)) == 0)
    11ac:	83 c4 10             	add    $0x10,%esp
    11af:	85 c0                	test   %eax,%eax
    11b1:	75 bd                	jne    1170 <malloc+0x50>
        return 0;
  }
}
    ...
    11ce:	89 4a 04             	mov    %ecx,0x4(%edx)
        p += p->s.size;
    11d1:	8d 14 ca             	lea    (%edx,%ecx,8),%edx
        p->s.size = nunits;
    11d4:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
    11d7:	a3 64 19 00 00       	mov    %eax,0x1964
}
    11dc:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return (void*)(p + 1);
    11df:	8d 42 08             	lea    0x8(%edx),%eax
}
    11e2:	5b                   	pop    %ebx
    11e3:	5e                   	pop    %esi
    11e4:	5f                   	pop    %edi
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    121a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
        prevp->s.ptr = p->s.ptr;
    1220:	8b 0a                	mov    (%edx),%ecx
    1222:	89 08                	mov    %ecx,(%eax)
